...
final case class PersistentExecutor(
  ...
  //TODO : this eval function will also take the workflow state.
  ...
  //TODO : def evalWithSchema(lookup : VariableLookup )

  //TODO : trait VariableLookup {
  //def lookUp[A] (a : Remote.Variable[A]): Either[String, A]
...
// }
  ...
  ////    } yield b
  //
  //    // 1. Read the environment `A` => ZFlow.PeekEnv (peek the environment)
  ...
  def submit[E: Schema, A: Schema](uniqueId: String, flow: ZFlow[Any, E, A]): IO[E, A] = {
    def step(ref: Ref[State[E, A]]): IO[IOException, Any] = {
      ref.get.flatMap { state =>
        state.current match {
              ...
              state.stack match {
                case Nil =>
                  eval(value).flatMap { schemaAndValue0 =>
                    ...
                    state.result.succeed(schemaAndValue.value: A)
                  }
                case k :: newStack =>
          ...
          case PeekEnv() =>
            ref.get.flatMap { state =>
              val env = state.currentEnvironment match {
...
//
//            }
            //TODO : create 2 variables - one for success, one for failure
            //feed those variables into the functions inside Fold
            //Return workflows that read from the environment.
            //
            putStrLn("Fold is submitted").provide(Has(console.Console.Service.live)) *>
              ref.update { state =>
                val env = (state.currentEnvironment match {
                  case Some(value) => value
                  case None => SchemaAndValue[Unit](Schema[Unit], ())
                }).unsafeCoerce[fold.ValueR]
          ...
          case fold@Fold1(_, _, _) =>
            ref.update { state =>
              state.copy(current = fold.value, stack = Continuation(fold.ifError, fold.ifSuccess) :: state.stack)
            } *> step(ref)
          ...
          case Ensuring(flow, finalizer) =>
            ref.get.flatMap { state =>
              ...
              ref.update(_.copy(current
                = flow, stack = cont :: state.stack)) *>
                step(ref)
            }
          ...
          case Provide(value, flow) =>
            eval(value).flatMap { schemaAndValue =>
              ref.update {
                state => state.pushEnv(schemaAndValue).copy(current = ZFlow.Ensuring(flow, ZFlow.PopEnv))
              } *> step(ref)
            }
            ...
            ref.get.flatMap { state =>
              state.stack match {
                ...
                case Nil =>
                  for {
                    err <- eval(error).map(_.value)
                    _ <- state.result.fail(err.asInstanceOf[E])
                  } yield ()
              }
            }
          ...
          //              state.stack match {
          //                case Nil =>
          //                  state.result.succeed(().asInstanceOf[A]).unit
          //
          //                case k :: newStack =>
          //                  ref.update(_.copy(current = ZFlow.PushEnv(Remote(())) *> ZFlow.Ensuring(k.onSuccess, ZFlow.PopEnv), stack = newStack)) *>
          //                    step(ref)
          //              }
          //            }
        }
...
object PersistentExecutor {
  ...
  final case class State[E, A](
                              ...
                              //TODO : Type Map[Remote.Variable[A], SchemaAndValue[A]]
                              //TODO : Type Map[String, Remote.Variable]
    ...
    def currentEnvironment: Option[SchemaAndValue[_]] = envStack.headOption

    def pushEnv(schemaAndValue: SchemaAndValue[_]): State[E, A] = copy(envStack = schemaAndValue :: envStack)

    def popEnv(): State[E, A] = envStack.headOption match {
      case Some(_) => copy(envStack = envStack.tail)
      case None => self
    }
  ...
  sealed trait TState {
    ...
    def enterTransaction(flow: ZFlow[Any, _, _]): TState =xz
      self match {
